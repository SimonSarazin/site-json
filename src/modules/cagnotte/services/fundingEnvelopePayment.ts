 ...
 *
 * Statut : REMPLACÉ
 * Raison : version "v1" qui faisait un `fetch("/co2/aap/fundingenvelope/")` HTTP brut.
 *   Le code a basculé sur l'appel SDK natif `entity.fundingEnvelope({...})` (méthode
 *   typée sur `BaseEntity`) qui passe par le SDK Cocolight et gère l'auth/transformation.
 *
 * Alternative active : `submitFundingEnvelopeAction` inline dans
 *   `src/components/cagnotte/PaymentConfigPage.tsx:195-232`.
 *
 * À reviewer : supprimer si l'approche SDK est définitive.
 */
