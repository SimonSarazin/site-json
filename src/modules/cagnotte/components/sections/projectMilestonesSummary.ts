 ...
 *
 * Statut : NON UTILISÉ
 * Raison : aucun consommateur dans le projet (vérifié par grep).
 *   Le type `NormalizedProjectMilestone` n'est référencé nulle part et
 *   `summarizeProjectMilestones` n'est jamais appelée.
 *
 * Alternative active : la summary des milestones est calculée inline dans
 *   `FinanceSection.tsx` et `FinanceSummarySection.tsx` à partir des données brutes.
 *
 * À reviewer : supprimer si pas de plan d'utilisation, ou brancher si une vue
 *   "résumé milestones standardisé" est prévue.
 */
