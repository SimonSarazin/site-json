...
function PaymentStatusBadge({ status }: { status: PaymentStatus }) {
  const config: Record<PaymentStatus, { icon: ReactNode; label: string; className: string }> = {
    ...
    failed: { icon: <CircleDot className="h-3 w-3" />, label: 'Échoué', className: 'bg-destructive/20 text-destructive' },
    refunded: { icon: <CircleDot className="h-3 w-3" />, label: 'Remboursé', className: 'bg-muted text-muted-foreground' },
  };
  const current = config[status];
  return (
...
export default function FinanceSection({ id, props }: { id?: string; props: FinanceSectionProps }) {
  ...
  // Contexte commun pour les mutations milestone (api + ids résolus).
  // Le `||''` garantit qu'on a toujours une string : la factory throw si elle est vide,
  // ce qui transforme l'erreur en toast `errorKey` via `useMutationWithToast`.
  ...
  // Hors profil, fallback sur l'entité Cocolight.
      ...
      <Dialog open={isEditMilestoneOpen} onOpenChange={setIsEditMilestoneOpen}>
        <DialogContent className="sm:max-w-xl border border-primary/20 bg-card p-0 overflow-hidden">
          <DialogHeader className="space-y-2 px-6 pt-6 pb-4 border-b bg-muted/30">
            <DialogTitle className="text-xl font-display">{t('FinanceSection.editDialog.title')}</DialogTitle>
          </DialogHeader>

          <form onSubmit={editForm.handleSubmit(onValidEditMilestone)} className="contents">
            <div className="space-y-4 px-6 py-5">
              <div className="space-y-2">
                <Label htmlFor="edit-milestone-name">{t('FinanceSection.editDialog.fields.name')}</Label>
                <Input
              ...
              <div className="space-y-2">
                <Label htmlFor="edit-milestone-description">{t('FinanceSection.editDialog.fields.description')}</Label>
                <Textarea
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div className="space-y-2">
                  <Label htmlFor="edit-milestone-target">{t('FinanceSection.editDialog.fields.targetAmount')}</Label>
                  <Input
                ...
                <div className="space-y-2">
                  ...
                  >
                    <SelectTrigger className="w-full">
                      <SelectValue placeholder={String(t('FinanceSection.editDialog.fields.statusPlaceholder'))} />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="open">{t('FinanceSection.editDialog.statusOptions.open')}</SelectItem>
                      <SelectItem value="done">{t('FinanceSection.editDialog.statusOptions.done')}</SelectItem>
                      <SelectItem value="close">{t('FinanceSection.editDialog.statusOptions.close')}</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
              </div>
              ...
              {isSubmittingEditMilestone ? (
                <Alert aria-live="polite" className="border-primary/20 bg-primary/5 text-primary">
                  <Loader2 className="h-4 w-4 animate-spin" />
                  <AlertDescription className="text-primary">{t('FinanceSection.editDialog.updatingState')}</AlertDescription>
                </Alert>
              ) : null}
            </div>
            ...
            <DialogFooter className="px-6 py-4 border-t bg-muted/20">
              ...
              <Button type="submit" disabled={isSubmittingEditMilestone} className="bg-primary hover:bg-primary/90">
                ...
                <CollapsibleContent className="pt-3 space-y-2.5">
                  {transactions.length > 0 ? (
                    transactions.map((transaction) => {
                      ...
                      return (
                        <div key={transaction.id} className={`rounded-lg border px-3 py-2.5 text-sm ${isPaid ? 'border-success/30 bg-success/10' : 'border-border/70 bg-muted/40'}`}>
                          <div className="flex items-start justify-between gap-2">
                            <div className="space-y-1">
                              <div className="flex items-center gap-1.5">
                                {isPaid && <CheckCircle2 className="h-4 w-4 text-success" />}
                            ...
                            <div className="flex flex-col items-end gap-1">
                              ...
                              <span className={`font-semibold ${isPaid ? 'text-success' : 'text-primary'}`}>{formatCurrency(transaction.amount)}</span>
                            </div>
                          </div>
                        </div>
                      );
