...
export function useCoFormCatalogs({
  ...
  const { data, isLoading, error, refetch } = useQuery({
    queryKey: COFORM_QUERY_KEYS.formCatalogs(formId, inputKeys),
    queryFn: async () => {
      if (!api) throw new Error("API non initialisée");
      const response = await api.endpointApi.getCoformCatalogs({
        formId,
      ...
      const raw = (response?.serverData?.data ?? response?.data) as CommonTableCatalogs | undefined;
