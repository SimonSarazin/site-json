import { Calendar, MapPin, Users, Mail, Phone, Globe, Briefcase, ChevronRight } from "lucide-react";
...
export default function ProfileInfo({ section }: ProfileInfoProps) {
  ...
  // Determine card classes based on variant and sticky
  ...
  return (
    <div className={cardClasses}>
      ...
      {section.showCounts !== false && (membersCount !== null || projectsCount !== null) && (
      ...
      <div className="space-y-4">
        {/* Username */}
        {section.showUsername !== false && username && typeof username === "string" && (
        ...
        {section.showPhone !== false && mobile && typeof mobile === "string" && (
          <div className="flex items-center gap-3 p-2 hover:bg-muted rounded-lg transition-colors">
            ...
            <a href={`tel:${mobile}`} className="text-foreground font-medium text-sm hover:text-teal-600">{mobile}</a>
          </div>
        )}
        ...
        {section.showWebsite !== false && url && typeof url === "string" && (
          <div className="flex items-center gap-3 p-2 hover:bg-muted rounded-lg transition-colors">
            ...
            <a href={url} target="_blank" rel="noopener noreferrer" className="text-teal-600 hover:text-teal-700 break-all text-sm font-medium">
              {url.replace(/^https?:\/\//, '')}
            </a>
          </div>
        )}
        ...
        {/* Address */}
        {section.showAddress !== false && address?.postalCode && address.addressLocality && (
        ...
        {/* Opening Date */}
        {section.showOpeningDate !== false && openingDate && (typeof openingDate === 'string' || typeof openingDate === 'number' || openingDate instanceof Date) && (
          <div className="flex items-center gap-3 p-2 hover:bg-muted rounded-lg transition-colors">
            <Calendar className="w-5 h-5 text-teal-600 shrink-0" />
            <span className="text-foreground text-sm font-medium">{t("ProfileTemplateDefault.openSince")} {formatDate(openingDate as string | number | Date)}</span>
          </div>
        )}

        {/* Event Dates (for events) */}
        {section.showDates !== false && startDate && (typeof startDate === 'string' || typeof startDate === 'number' || startDate instanceof Date) && (
          <div className="flex items-start gap-3 p-2 hover:bg-muted rounded-lg transition-colors">
            <Calendar className="w-5 h-5 text-teal-600 mt-0.5 shrink-0" />
            <div>
              <p className="font-medium text-foreground text-sm">{t("common.date")}</p>
              <p className="text-sm text-foreground">
                {formatDate(startDate as string | number | Date)}
                {endDate && (typeof endDate === 'string' || typeof endDate === 'number' || endDate instanceof Date) && (
        ...
        {section.showOrganizer !== false && Object.keys(organizers).length > 0 && (
          <div className="flex items-start gap-3 p-2 hover:bg-muted rounded-lg transition-colors">
            ...
            <div>
              <p className="font-medium text-foreground text-sm">{t("common.organizedBy")}</p>
        ...
        {/* Registration Button */}
        {externalLinkRegistration && typeof externalLinkRegistration === "string" && (
          <div className="mt-6 pt-6 border-t border-border">
            <Button
              className="w-full bg-[#0092a2] hover:bg-teal-600"
              onClick={() => window.open(externalLinkRegistration, "_blank")}
            >
