...
export const verifyHelloAssoSignature = async (
  ...
  try {
    ...
    console.log("🔐 Vérification signature HelloAsso...");
...
export const processHelloAssoWebhook = async (
  ...
  try {
    console.log("🔔 Traitement webhook HelloAsso:", webhookData.id);
    ...
    if (webhookData.status === "completed") {
      console.log("✅ Paiement HelloAsso confirmé:", webhookData.id);
      return {
        isValid: true,
        status: "success",
      ...
      console.log("⏳ Paiement HelloAsso en attente:", webhookData.id);
      return {
        isValid: false,
        status: "pending",
        transactionId: webhookData.id,
      ...
      console.log("❌ Paiement HelloAsso échoué:", webhookData.id);
      return {
        isValid: false,
        status: "failed",
        transactionId: webhookData.id,
      ...
      console.log("⚠️ Statut paiement inconnu:", webhookData.status);
      return {
        isValid: false,
        status: "error",
        transactionId: webhookData.id,
...
export const verifyHelloAssoPaymentStatus = async (
  ...
  try {
    console.log("🔍 Vérification statut paiement HelloAsso:", transactionId);
    ...
    console.warn("⚠️ Vérification API HelloAsso non encore implémentée");
    return {
      isValid: false,
      status: "pending",
      transactionId,
