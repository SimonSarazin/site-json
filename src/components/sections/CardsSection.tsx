              ...
              <CardWrapper key={index} href={item.href} target={item.target}>
                <div className="relative w-full h-64 sm:h-80 md:h-96 rounded-xl overflow-hidden shadow-lg group cursor-pointer">
                  ...
                  <div className="absolute top-2 sm:top-3 left-2 sm:left-3 flex flex-col gap-1 sm:gap-2 z-10">
                    {item.date && (
                      <div className="bg-card w-auto px-2 sm:px-3 py-0.5 sm:py-1 rounded-md text-[10px] sm:text-xs font-semibold shadow text-card-foreground flex items-center gap-1">
                  ...
                  <div className="absolute bottom-2 sm:bottom-3 left-2 sm:left-3 right-2 sm:right-3 bg-card/90 rounded-xl p-2 sm:p-4 flex items-start gap-2 sm:gap-3 shadow-lg transform translate-y-2 group-hover:translate-y-0 transition-all">
                    {item.avatarIcon && (
                      <div className={cn(
                    ...
                    <div className="flex-1 min-w-0">
                      <h3 className="font-bold text-foreground text-xs sm:text-sm mb-0.5 sm:mb-1 truncate">
                        <T k={item.organizerName || item.title} />
                      </h3>
                      {item.location && (
                        <T k={item.location} as="p" className="text-muted-foreground text-[10px] sm:text-xs truncate" />
                      )}
                    </div>
                  </div>
                </div>
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {currentLayout === 'list' ? (
            <div className="space-y-3 sm:space-y-4">
              {items.map((item, index) => (
                <CardWrapper key={index} href={item.href} target={item.target}>
                  <div className="relative bg-card rounded-xl overflow-hidden shadow hover:shadow-lg transition cursor-pointer">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-start gap-3 mb-2">
                          ...
                          <div className="flex-1">
                            <T k={item.title} as="h3" className="font-bold text-foreground text-base mb-1" />
                            {item.location && (
                              <T k={item.location} as="p" className="text-muted-foreground text-sm" />
                            )}
                          </div>
                          ...
                          {item.badges && item.badges.length > 0 && (
                            <div className="flex gap-2">
                              {item.badges.map((badge, idx) => (
                                <button
                                  key={idx}
                                  className="w-8 h-8 bg-muted rounded-full shadow-sm flex items-center justify-center hover:bg-muted/80 transition"
              ...
              {items.map((item, index) => (
                <CardWrapper key={index} href={item.href} target={item.target}>
                  <div className="relative bg-card rounded-xl overflow-hidden shadow hover:shadow-lg transition group cursor-pointer">
                    ...
                    {item.badges && item.badges.length > 0 && (
                      <div className="absolute top-3 right-3 flex gap-2 z-10">
                        {item.badges.map((badge, idx) => (
                          <button
                            key={idx}
                            className="w-8 h-8 bg-card rounded-full shadow flex items-center justify-center hover:bg-muted transition"
                            aria-label={badge.label}
                          >
                            <DynamicIcon name={badge.icon as IconName} className="w-4 h-4 text-muted-foreground" />
                          </button>
                    ...
                    <div className="absolute bottom-3 left-3 right-3 bg-card rounded-xl p-3 px-4 mb-2 flex items-start gap-3 shadow-lg">
                      {item.avatarIcon && (
                        <div className={cn(
                          "w-8 h-8 flex items-center justify-center rounded-full shadow-sm flex-shrink-0",
                          getAvatarColorClasses(item.avatarColor)
                      ...
                      <div>
                        <T k={item.title} as="h3" className="font-bold text-foreground text-sm mb-1" />
                        {item.location && (
                          <T k={item.location} as="p" className="text-muted-foreground text-xs" />
                        )}
                      </div>
                    </div>
                  </div>
            ...
            {items.map((item, index) => (
              <CardWrapper key={index} href={item.href} target={item.target}>
                <div className="group cursor-pointer bg-card rounded-2xl shadow-md hover:shadow-xl transition p-6 border border-border hover:border-primary">
                  ...
                  <T k={item.title} as="h4" className="font-bold mb-2 text-foreground text-sm group-hover:text-primary transition-colors text-center" />

                  <T k={item.text} as="p" className="text-xs text-muted-foreground line-clamp-2 text-center" />
                </div>
              </CardWrapper>
            ))}
          </div>
