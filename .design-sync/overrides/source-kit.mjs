...
// forked from design-sync lib/source-kit.mjs — groupes `module-<m>` pour src/modules/* (les noms de dossiers parts/shared/custom sont muets dans le picker)
...
export async function resolvePackage(ctx) {
  ...
  if (srcRoot) {
    for (const c of components) {
      ...
      const relSegs = slash(relative(srcRoot, dirname(hit))).split('/');
      c.group = slug(
        (relSegs[0] === 'modules' && relSegs[1] ? `module-${relSegs[1]}` : null)
        || relSegs
