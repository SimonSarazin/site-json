...
app.post("/api/admin/sites", (req, res) => {
  ...
  const config = JSON.parse(fs.readFileSync(configPath, "utf-8"));
  siteRegistry.set(slug, {
    config, configPath,
    configScript: `<script>window.__CONFIG__=${serialize(config, { isJSON: true })}</script>`,
    meta: { slug, config: configFile, css },
  });
...
app.post('/api/admin/config-save', (req, res) => {
  const saveSlug = req.body?._slug;
  try {
    if (isMultiSite && saveSlug) {
      const entry = siteRegistry.get(saveSlug);
      if (!entry) return res.status(404).json({ error: `Slug "${saveSlug}" non trouvé` });
      const { _slug, ...configData } = req.body;
      fs.writeFileSync(entry.configPath, JSON.stringify(configData, null, 2) + "\n", "utf-8");
      entry.config = configData;
      entry.configScript = `<script>window.__CONFIG__=${serialize(configData, { isJSON: true })}</script>`;
      console.log(`[Admin] Config saved for "${saveSlug}"`);
      res.json({ ok: true });
    } else {
      if (!process.env.SITE_CONFIG_PATH) return res.status(400).json({ error: "SITE_CONFIG_PATH non défini" });
      const envPath = process.env.SITE_CONFIG_PATH;
      const configPathResolved = path.isAbsolute(envPath) ? envPath : path.resolve(process.cwd(), envPath);
      fs.writeFileSync(configPathResolved, JSON.stringify(req.body, null, 2) + "\n", "utf-8");
      cachedConfig = req.body;
      configScript = `<script>window.__CONFIG__=${serialize(cachedConfig, { isJSON: true })}</script>`;
      console.log("[Admin] Config saved to", configPathResolved);
      res.json({ ok: true });
    }
  } catch (e) {
