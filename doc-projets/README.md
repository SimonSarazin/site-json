...
| Ekilib.re — Maison Sport Santé du Tampon | `associationEkilibre` | [maison-sport-sante-la-tampon.md](maison-sport-sante-la-tampon.md) | Sous-site territorial du costum SSBE (CP 97430/97418). Vitrine + annuaires créneaux/structures ✅ (11 pages, audit 0 constat), fiche détail structure + back-office/KPIs CDC 4.2 commités le 06/08. **Actualités REFONDÉES post-review MR 44 (21/08)** sur le module Articles/Blog (articleFeed épinglage "flag", reader /blog/:slug, RSS) + publicationStatus au patron statusField (Brouillon défaut) + chaîne costumSlug updateField (multi-admin) ; nœud typeObj.article posé en dev (à rejouer en PROD, cf. mss-la-tampon-plan-corrections.md). Restent : miroir prod (3 scripts), release lib, contact cassé (`/api/contact`), déploiement |
