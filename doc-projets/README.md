...
| Ekilib.re — Maison Sport Santé du Tampon | `associationEkilibre` | [maison-sport-sante-la-tampon.md](maison-sport-sante-la-tampon.md) | Sous-site territorial du costum SSBE (CP 97430/97418). Vitrine + annuaires créneaux/structures ✅ (11 pages, audit 0 constat), fiche détail structure + back-office/KPIs CDC 4.2 commités le 06/08. **Nouveau chantier annoncé (non codé)** : actualités de la home reportées en CRUD POI (remplace News). Restent : MR `ekilibre`→`main`, contact cassé (`/api/contact`), recette navigateur, déploiement |
...
| Module Messagerie — **module transversal du moteur** (pas un site) | `messagerie` (module) | [module-messagerie.md](module-messagerie.md) | Cadrage du 31/07 : existant inventorié dans les 2 dépôts (0 endpoint SDK sur 141, aucun temps réel), architecture, suggestions phasées V1/V2/V3 et **10 fiches d'endpoints prêtes pour ENDPOINT.md**, phasage en 6 lots. **0 ligne de code** — en attente des arbitrages (temps réel, modèle de données, portée multi-tenant, mailer, site pilote) |
