...
| Fédération des CAE — Appel à Communs | `federationDesCae` | [federation-des-cae.md](federation-des-cae.md) | **Branche `aac-dev`** — 3 pages (accueil, annuaire des communs, catalogue des besoins). Dossier **extrait de `tiers-lieux.md` le 31/08** : le travail AAC n'a rien à voir avec le site des tiers-lieux. Catalogue d'usages à 83 outils ; règle de satisfaction et sous-catégorie corrigées côté backend (31/08). Restent : le **logo manquant** (seul constat d'audit et préflight rouge), le bloc « Informations liées au commun » réduit sur un commun de cet appel, la version mobile du catalogue, et **aucune vérification navigateur** |
