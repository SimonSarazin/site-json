...
|---|---|---|---|
| Réseau Parentalité 62 | `parent62` | [parent62.md](parent62.md) | Partie 1 en cours — 48 pages / 178 sections. Lots du 09/09 : paroles de parents (image, adresse, carte), page `/theme`, 9 pages Publics dont 2 neuves ; **déploiement à faire**, `test:e2e` à rejouer |
...
| Fédération des CAE — Appel à Communs | `federationDesCae` | [federation-des-cae.md](federation-des-cae.md) | **Sur `main` depuis le 09/09** (merge `287bb35a`) — 3 pages (accueil, annuaire des communs, catalogue des besoins). Dossier **extrait de `tiers-lieux.md` le 31/08**. Le **module AAC** (MR !53) a été relu puis corrigé en **112 commits** : 4 bloquants dont 3 pertes de données, décisions tranchées avec le backend (`form.coremu`, `priceInt` calculé et non stocké, une étape absente du payload est préservée), et une régression critique introduite par les correctifs eux-mêmes puis rattrapée. Logo enfin livré, détouré et transparent. Restent : le financement **masqué** tant que `form.coremu` n'est pas activé côté legacy, le **500 de `getformbyid`** à déployer avant la prod, `test:integration`/`test:e2e` jamais lancés |
...
| RéseauSanté — Rézo Santé Réunion | `EcosystemeSanteReunion` | [rezo-sante-reunion.md](rezo-sante-reunion.md) | **15 pages, 44 sections, audit vert**. **Slug du costum renommé en prod le 08/09** (ex-`rezoSanteReunion`) : répercuté sur toute la config et les fixtures (§9 lot 6). **Création de fiches livrée le 08/09** (§9 lot 5) : 3 formulaires costum — annuaire, projets, ressources — avec les champs Thématiques et Bénéficiaires, adossés au `typeObj` du costum posé en base (§6bis.1) ; reste à appliquer ce `typeObj` au backend de déploiement. Périmètres du costum **toujours vides** → pages de recherche désertes tant que rien n'est peuplé. Les 5 actions d'engagement demandées (like/partenaire/soutien/intéressé/réutilise) et le partenariat inter-orga **n'existent pas dans le produit** — chiffrage en 3 couches dans la doc |
...
| Réseau KPA-Cité | `kpacite` | [kpacite.md](kpacite.md) | Configuration initiale : accueil, annuaire de 6 KPAs et diagnostic collectif CoForm. Validation, audit et probe verts ; rendu SSR et déploiement à faire |
