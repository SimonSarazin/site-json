...
| RéseauSanté — Rézo Santé Réunion | `rezoSanteReunion` | [rezo-sante-reunion.md](rezo-sante-reunion.md) | **15 pages, 44 sections, audit vert** (lot 4, 27/08 : 6ᵉ thématique « Culture & Santé » + correctif de lisibilité des héros `hero-tinted-overlay`, en working tree non commité). Costum backend **créé le 30/07** (config minimale, §6bis) mais périmètres toujours vides → pages de recherche désertes tant que rien n'est peuplé, tag de la 6ᵉ thématique non mesuré. Feuille CSS propre + titres Fraunces (30/07), hero-carousel en **essai** sur /thematiques. Les 5 actions d'engagement demandées (like/partenaire/soutien/intéressé/réutilise) et le partenariat inter-orga **n'existent pas dans le produit** — chiffrage en 3 couches dans la doc |
