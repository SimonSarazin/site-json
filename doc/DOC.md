...
### 14.1 Dockerfile — Build multi-stage
...
**Etape 2 (runner)** : image de production legere contenant uniquement le build et les dependances runtime.
...
### 14.2 Variables d'environnement runtime
...
|--------|--------------------------|-------------|
| Config JSON | `/app/config.prod.json` (ou autre chemin) | Fichier de configuration du site, monte depuis l'hote. Doit correspondre a `SITE_CONFIG_PATH`. |
| Cache images | `/app/.cache/images` | Cache des images optimisees par le middleware sharp. Persiste entre les redemarrages pour eviter de re-optimiser. |
| Images de contenu | `/app/dist/client/images/<dossier>` | Images referencees dans la config JSON mais absentes du build (voir section suivante). |

### 14.4 Ajouter des images de contenu en production
...
### 14.5 Exemple complet docker-compose.yml
...
services:
  cocostum:
    ...
    volumes:
      ...
      - ./config.prod.tiers-lieux.json:/app/config.prod.json
      # Images de contenu (si besoin d'images non presentes dans le build)
      - ./images/franceTierslieux:/app/dist/client/images/franceTierslieux
...
### 14.6 Recapitulatif du flux build / runtime
