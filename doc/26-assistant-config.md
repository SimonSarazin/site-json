...
- `src/types/site-schema.ts` (~2 376 lignes) : `SiteConfig` racine, `Header`,
  `Footer`, `Page`, et **78 sections** discriminées par
...
- Les schémas n'ont **aucun `.describe()`** aujourd'hui — mais les commentaires
  ...
  des **78 sections** pour le panel) fournissent la matière sémantique à injecter
...
- **Sections** : `src/components/admin/section-meta.ts` — **78 sections** avec
